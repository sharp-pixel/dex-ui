...
void Radar::compute_lines_from_bezier() {
	...
	for (int i = 0; i < bezier_segments.size(); i++) {
		...
		std::vector<ofPoint> points = bezier_to_points(b);
...
ofPoint rec_de_casteljau(std::vector<ofPoint> points, float t) {
	...
	std::vector<ofPoint> intermediates;
...
std::vector<ofPoint> bezier_to_points(bezier b) {
