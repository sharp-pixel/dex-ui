...
TimeDisplay::TimeDisplay() {
	...
	auto upperStrings = std::array<std::string, 4>{year_s, "UPTIME", "SYSTEM", "V"};
	auto lowerStrings = std::array<std::string, 4>{mon_s, uptime_s, "ONLINE", "0.1b"};
	auto xpositions = std::array<int, 4>{0 + inset, 4 * GRID_SIZE + inset, 9 * GRID_SIZE + inset, 14 * GRID_SIZE + inset};

	// Upper
	for (int i = 0; i < 4; i++) {
		auto newT = newText(upperStrings[i], 7,
	...
	// Lower
	for (int i = 0; i < 4; i++) {
		auto newT = newText(lowerStrings[i], 7,
...
void TimeDisplay::updateClockTime() {
	...
	// Update time strings
	hour_s = ofToString(localizedTime->tm_hour, 2, '0');
	min_s = ofToString(localizedTime->tm_min, 2, '0');
	sec_s = ofToString(localizedTime->tm_sec, 2, '0');

	uptime_s = ofToString((int)(diff_sec / 3600) % 99, 2, '0');
	uptime_s += ":" + ofToString((int)(diff_sec / 60) % 60, 2, '0');
	uptime_s += ":" + ofToString((int)diff_sec % 60, 2, '0');
}
